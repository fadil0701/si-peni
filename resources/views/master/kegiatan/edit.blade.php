...
<div class="bg-white shadow-sm rounded-lg border border-gray-200">
    <div class="px-6 py-5 border-b border-gray-200">
        <h2 class="text-xl font-semibold text-gray-900">Edit Kegiatan</h2>
    </div>
    
    <form action="{{ route('master.kegiatan.update', $kegiatan->id_kegiatan) }}" method="POST" class="p-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
            <div>
                ...
                <select id="id_program" name="id_program" required class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('id_program') border-red-500 @enderror">
                    ...
                    @foreach($programs as $program)
                        <option value="{{ $program->id_program }}" {{ old('id_program', $kegiatan->id_program) == $program->id_program ? 'selected' : '' }}>{{ $program->nama_program }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" id="nama_kegiatan" name="nama_kegiatan" required value="{{ old('nama_kegiatan', $kegiatan->nama_kegiatan) }}" placeholder="Masukkan nama kegiatan" class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('nama_kegiatan') border-red-500 @enderror">
